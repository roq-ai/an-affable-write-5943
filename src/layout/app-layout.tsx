...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFeather } from 'react-icons/fi';

import { AuthorInterface } from 'interfaces/author';
import { getAuthors } from 'apiSdk/authors';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['author', 'editor', 'marketing-manager', 'reader'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AuthorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/authors` : null),
    () => getAuthors({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Authors',
      path: isTenantUser ? `/authors/view/${data?.[0]?.id}` : '/authors',
      entity: 'author',
