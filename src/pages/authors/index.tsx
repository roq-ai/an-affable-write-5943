...
import { getAuthors, deleteAuthorById } from 'apiSdk/authors';
import { AuthorInterface } from 'interfaces/author';

type ColumnType = ColumnDef<AuthorInterface, unknown>;
...
export function AuthorListPage(props: AuthorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AuthorInterface>>(
    () => `/authors?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AuthorInterface) => {
    if (hasAccess('author', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/authors/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/authors/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('author', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/authors/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('author', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
