...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'content_published.create': {
    roles: ['author', 'editor', 'marketing-manager'],
    key: 'content',
    tenantPath: [],
    userPath: [],
  },
...
const tenantRoles: string[] = ['author', 'editor', 'marketing-manager', 'reader'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.author.findFirst({
